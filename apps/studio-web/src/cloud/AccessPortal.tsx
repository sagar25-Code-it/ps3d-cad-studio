  ...
  return <main className="public-page access-page">
    ...
    {session !== undefined && !recoveryMode && <section className="account-console">
      ...
      <div className="account-grid">
        ...
        <div className="endpoint-card"><span className="step-index">02</span><h3>Choose a client connection</h3><dl><div><dt>Remote MCP endpoint</dt><dd><code>{config?.mcpEndpoint ?? `${location.origin}/api/mcp`}</code></dd></div><div><dt>Read-only inspection</dt><dd>OAuth 2.1 when your AI client supports automatic authorization</dd></div><div><dt>Preview or confirmed copy</dt><dd>A separately scoped personal bearer token for clients that accept custom HTTP headers</dd></div></dl><a href="/learn#mcp">Open the model-by-model connection guide</a></div>
      </div>
