...
import { CAD_COMMANDS, auditCadCommandSurface, type CapabilityLevel, type WorkspaceId } from "../../../../packages/workbench-core/src/index.js";
...
export function CommandAuditPage(): React.JSX.Element {
  ...
  const audit = useMemo(() => auditCadCommandSurface(), []);
  ...
  return <main className="command-audit-page">
    ...
    <section className="audit-summary" aria-label="Command capability summary"><div><small>Total catalog</small><strong>{audit.total}</strong></div><div><small>Executable contracts</small><strong>{audit.executable}</strong></div><div><small>Truthfully blocked</small><strong>{audit.truthfullyBlocked}</strong></div><div><small>Contract issues</small><strong>{audit.issues.length}</strong></div></section>
    <section className="audit-method"><div><CommandIcon name="shield" /><span><strong>No copied product code or UI assets</strong><small>Command semantics were independently implemented or documented from public professional-CAD workflows.</small></span></div><div><CommandIcon name="cube-check" /><span><strong>Every catalog entry is machine-audited</strong><small>{audit.actionKindsCovered.length} executable action kinds have a registered UI handler contract; stable IDs, shortcuts, discovery text, guides, and capability labels are checked together.</small></span></div><div><CommandIcon name={audit.passed ? "cube-check" : "cancel"} /><span><strong>{audit.passed ? "Command surface audit passed" : "Command surface audit needs correction"}</strong><small>{audit.passed ? "No duplicate ID, shortcut collision, missing trial contract, label mismatch, or unregistered action handler was found." : audit.issues.map((issue) => `${issue.commandId}: ${issue.message}`).join(" ")}</small></span></div></section>
    ...
    <section className="audit-command-grid" aria-label="Audited command cards">
      {commands.map((command) => <article key={command.id} className={`audit-command ${command.level}`} data-workspace={command.workspace}>
        ...
        <footer><code>{command.id}</code><span>{command.action.kind === "unavailable" ? "Blocked boundary verified" : `Handler: ${command.action.kind}`}{command.shortcut === undefined ? "" : ` · ${command.shortcut}`}</span></footer>
      </article>)}
    </section>
